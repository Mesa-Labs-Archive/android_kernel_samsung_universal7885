...
{
	pr_info("%s: before base=%pa, size=%pa\n", __func__, &abox_rmem->base, &abox_rmem->size);

	free_reserved_area((void *)(phys_to_virt(abox_rmem->base) + MIN_ABOX_DBG_RMEM_SZ),
				(void *)((void *)(p_abox_dbg_dump) + abox_rmem->size), 0, "abox_rmem");

	abox_rmem->size = MIN_ABOX_DBG_RMEM_SZ;

	p_abox_dbg_dump_min = (void *) p_abox_dbg_dump;
	p_abox_dbg_dump = NULL;
