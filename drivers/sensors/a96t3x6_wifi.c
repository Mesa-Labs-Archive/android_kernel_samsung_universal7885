		...
		u8 reg, u8 *val, unsigned int len)
{
	...
	while (retry--) {
		...
		SENSOR_ERR("[WIFI]fail(address set)(%d)(%d)\n", retry, ret);
	...
	while (retry--) {
		...
		SENSOR_ERR("[WIFI]fail(data read)(%d)(%d)\n", retry, ret);
		usleep_range(10000, 11000);
	}
	mutex_unlock(&data->lock);
	return ret;
...
static int a96t3x6_i2c_read_data(struct i2c_client *client, u8 *val, unsigned int len)
{
	...
	while (retry--) {
		...
		SENSOR_ERR("[WIFI]fail(data read)(%d)\n", retry);
		usleep_range(10000, 11000);
	}
	mutex_unlock(&data->lock);
	return ret;
...
static void a96t3x6_set_enable(struct a96t3x6_data *data, bool enable, bool force)
{
	...
	if ((data->current_state == enable) || (!force && data->earjack) ||
		...
		SENSOR_INFO("[WIFI]old enable = %d, new enable = %d, skip exception case\n",
	...
	if (enable) {
		...
		if (ret < 0)
			SENSOR_ERR("[WIFI]failed to enable grip irq\n");
		enable_irq(data->irq);
		...
		ret = a96t3x6_i2c_write(data->client, REG_SAR_ENABLE, &cmd);
		if (ret < 0)
			SENSOR_ERR("[WIFI]failed to disable grip irq\n");
	}
	data->current_state = enable;
}
...
sar_mode:
	while (retry > 0) {
		...
		if (ret < 0) {
			SENSOR_ERR("[WIFI]i2c read fail(%d), retry %d\n", ret, retry);
			retry--;
	...
	if (ret < 0)
		SENSOR_ERR("[WIFI]i2c read fail(%d)\n", ret);
...
static void a96t3x6_sar_sensing(struct a96t3x6_data *data, int on)
{
	...
	if (ret < 0)
		SENSOR_ERR("[WIFI]failed to %s grip sensing\n", (on) ? "enable" : "disable");
}
		 ...
		 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret != 1) {
		SENSOR_ERR("[WIFI]cmd read err\n");
		return count;
	}

	if (!(force >= 0 && force <= 3)) {
		SENSOR_ERR("[WIFI]wrong command(%d)\n", force);
		return count;
	}
	...
	if (force == 3) {
		...
		SENSOR_INFO("[WIFI]Power back off force off -> on (%d)\n", data->current_state);
		if (!data->current_state)
			force = 1;
		else
			return count;
	...
	if (data->sar_enable_off) {
		if (force == 1)
			...
			enable = false;
		SENSOR_INFO("[WIFI]skip, Power back off force off mode (%d)\n", enable);
		return count;
	}
		 ...
		 size_t count)
{
	...
	if (ret != 1) {
		SENSOR_ERR("[WIFI]failed to read thresold, buf is %s\n", buf);
		return count;
	}
		 ...
		 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		SENSOR_ERR("[WIFI]kstrtou8 failed.(%d)\n", ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		SENSOR_ERR("[WIFI]Invalid value.(%d)\n", onoff);
	}
	mutex_unlock(&data->lock);
...
static int a96t3x6_load_fw_kernel(struct a96t3x6_data *data)
{
	...
	if (ret) {
		SENSOR_ERR("[WIFI]request_firmware fail.\n");
		return ret;
	}
...
static int a96t3x6_load_fw(struct a96t3x6_data *data, u8 cmd)
{
	...
	case SDCARD:
		...
		if (!data->firm_data_ums) {
			SENSOR_ERR("[WIFI]fail to kzalloc for fw\n");
			ret = -ENOMEM;
		...
		if (nread != fsize) {
			SENSOR_ERR("[WIFI]fail to vfs_read file\n");
			ret = -EINVAL;
...
static int a96t3x6_i2c_read_checksum(struct a96t3x6_data *data)
{
	...
	SENSOR_INFO("[WIFI]ret:%d [%X][%X][%X][%X][%X]\n", ret,
						...
						unsigned char *addrL, unsigned char *val)
{
	...
	if (ret != length) {
		SENSOR_ERR("[WIFI]write fail[%x%x], %d\n", *addrH, *addrL, ret);
		return ret;
	}
...
static int a96t3x6_fw_mode_enter(struct a96t3x6_data *data)
{
	...
	if (data->firmup_cmd != cmd) {
		SENSOR_ERR("[WIFI]cmd not matched, firmup fail (ret = %d)\n", ret);
		return -2;
	}
...
static int a96t3x6_fw_update(struct a96t3x6_data *data, u8 cmd)
{
	...
	for (i = 1; i < count; i++) {
		...
		if (ret < 0) {
			SENSOR_ERR("[WIFI]err, no device : %d\n", ret);
			return ret;
		}
	...
	while (retry--) {
		...
		if (cmd == BUILT_IN) {
			if ((data->checksum_h != data->checksum_h_bin) ||
				...
				SENSOR_ERR("[WIFI]checksum fail.(0x%x,0x%x),(0x%x,0x%x) retry:%d\n",
		...
		if (ret) {
			SENSOR_ERR("[WIFI]fw version read fail\n");
			ret = -1;
			continue;
		}

		if (data->fw_ver == 0) {
			SENSOR_ERR("[WIFI]fw version fail (0x%x)\n", data->fw_ver);
			ret = -1;
			continue;
		}

		if ((cmd == BUILT_IN) && (data->fw_ver != data->fw_ver_bin)) {
			SENSOR_ERR("[WIFI]fw version fail 0x%x, 0x%x\n",
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cmd == BUILT_IN) {
		...
		if (ret) {
			SENSOR_ERR("[WIFI]failed to load firmware(%d)\n", ret);
			goto fw_update_out;
		} else {
	...
	enable_irq(data->irq);
	if (ret) {
		SENSOR_ERR("[WIFI]failed to flash firmware(%d)\n", ret);
		data->fw_update_state = 2;
	} else {
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%4x,%4x", &regist, &val) != 2) {
		SENSOR_ERR("[WIFI]%s - The number of data are wrong\n", __func__);
		return -EINVAL;
	}
...
static int a96t3x6_fw_check(struct a96t3x6_data *data)
{
	...
	ret = a96t3x6_get_fw_version(data, true);
	if (ret)
		SENSOR_ERR("[WIFI]i2c fail(%d), addr[%d]\n", ret, data->client->addr);

	ret = a96t3x6_load_fw_kernel(data);
	if (ret)
		SENSOR_ERR("[WIFI]failed load_fw_kernel(%d)\n", ret);
	else
		SENSOR_INFO("[WIFI]fw version read success (%d)\n", ret);

	if (data->md_ver != data->md_ver_bin) {
		SENSOR_ERR("[WIFI]MD version is different.(IC %x, BN %x). Do force FW update\n",
	...
	if (data->fw_ver < data->fw_ver_bin || data->fw_ver > 0xa0
				|| force == true) {
		SENSOR_ERR("[WIFI]excute fw update (0x%x -> 0x%x)\n",
			data->fw_ver, data->fw_ver_bin);
		ret = a96t3x6_flash_fw(data, true, BUILT_IN);
		if (ret)
			SENSOR_ERR("[WIFI]failed to a96t3x6_flash_fw (%d)\n", ret);
		else
...
static int a96t3x6_power_onoff(void *pdata, bool on)
{
	...
	if (data->ldo_en) {
		...
		SENSOR_INFO("[WIFI][WIFI] ldo_en power %d\n", on);
	}

	data->dvdd_vreg = regulator_get(NULL, "vtouch_2.8v");
	if (IS_ERR(data->dvdd_vreg)) {
		data->dvdd_vreg = NULL;
		SENSOR_ERR("[WIFI]dvdd_vreg get error, ignoring\n");
	}
	...
	if (on) {
		if (data->dvdd_vreg) {
			...
			if (ret) {
				SENSOR_ERR("[WIFI]dvdd reg enable fail\n");
				return ret;
			}
		}
	} else {
		if (data->dvdd_vreg) {
			...
			if (ret) {
				SENSOR_ERR("[WIFI]dvdd reg disable fail\n");
				return ret;
			}
		}
	}
			...
			struct a96t3x6_data *data)
{
	...
	if (ret < 0) {
		SENSOR_ERR("[WIFI]gpio %d request failed (%d)\n", data->grip_int, ret);
		return ret;
	}

	ret = gpio_direction_input(data->grip_int);
	if (ret < 0) {
		SENSOR_ERR("[WIFI]failed to set direction input gpio %d(%d)\n",
				data->grip_int, ret);
...
static int a96t3x6_parse_dt(struct a96t3x6_data *data, struct device *dev)
{
	...
	if (ret < 0) {
		SENSOR_ERR("[WIFI]failed to read fw_path %d\n", ret);
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		SENSOR_ERR("[WIFI]i2c_check_functionality fail\n");
		return -EIO;
	}

	data = kzalloc(sizeof(struct a96t3x6_data), GFP_KERNEL);
	if (!data) {
		SENSOR_ERR("[WIFI]Failed to allocate memory\n");
		ret = -ENOMEM;
		goto err_alloc;
	}
	...
	if (!input_dev) {
		SENSOR_ERR("[WIFI]Failed to allocate memory for input device\n");
		ret = -ENOMEM;
	...
	if (ret) {
		SENSOR_ERR("[WIFI]failed to a96t3x6_parse_dt\n");
		goto err_config;
	}

	ret = a96t3x6_irq_init(&client->dev, data);
	if (ret) {
		SENSOR_ERR("[WIFI]failed to init reg\n");
		goto pwr_config;
	}
	...
	if (ret) {
		SENSOR_ERR("[WIFI]failed to firmware check (%d)\n", ret);
	...
	if (ret) {
		SENSOR_ERR("[WIFI]failed to register input dev (%d)\n",
			ret);
		goto err_reg_input_dev;
	}
	...
	ret = sensors_create_symlink(&data->input_dev->dev.kobj,
					data->input_dev->name);
	if (ret < 0) {
		SENSOR_ERR("[WIFI]Failed to create sysfs symlink\n");
	...
	if (ret < 0) {
		SENSOR_ERR("[WIFI]Failed to create sysfs group\n");
	...
	ret = sensors_register(&data->dev, data, grip_sensor_attributes,
				MODULE_NAME);
	if (ret) {
		SENSOR_ERR("[WIFI]could not register grip_sensor(%d)\n", ret);
	...
	if (ret < 0) {
		SENSOR_ERR("[WIFI]Failed to register interrupt\n");
		goto err_req_irq;
	}
	data->irq = client->irq;
	data->dev = &client->dev;
