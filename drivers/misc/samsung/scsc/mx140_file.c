...
int mx140_basedir_file(struct scsc_mx *mx)
{
	...
	strncpy(exe_dir, MX140_EXE_DIR_SYSTEM, sizeof(exe_dir));
	 ...
	 * If it's present, assume it'll contain our FW
	 */
	r = vfs_stat(MX140_FW_BASE_DIR_VENDOR_ETC_WIFI, &stat);
	if (r != 0) {
		SCSC_TAG_ERR(MX_FILE, "Base dir: %s doesn't exist\n", MX140_FW_BASE_DIR_VENDOR_ETC_WIFI);
		base_dir[0] = '\0';
		r = -ENOENT;
	} else {
		strncpy(base_dir, MX140_FW_BASE_DIR_VENDOR_ETC_WIFI, sizeof(base_dir));
		fw_base_dir = MX140_FW_BASE_DIR_VENDOR_ETC_WIFI;
		strncpy(exe_dir, MX140_EXE_DIR_VENDOR, sizeof(exe_dir));
		goto done;
	}

	/* Try /system partition (pre-Oreo) */
	r = vfs_stat(MX140_FW_BASE_DIR_SYSTEM_ETC_WIFI, &stat);
	if (r != 0) {
		SCSC_TAG_ERR(MX_FILE, "Base dir: %s doesn't exist\n", MX140_FW_BASE_DIR_SYSTEM_ETC_WIFI);
		base_dir[0] = '\0';
		r = -ENOENT;
	} else {
		strncpy(base_dir, MX140_FW_BASE_DIR_SYSTEM_ETC_WIFI, sizeof(base_dir));
