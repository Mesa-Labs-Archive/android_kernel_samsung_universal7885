				...
				enum power_supply_property psp)
{
	...
	if (!charger->pdata->load_switch_control) {
		return;
	} else if (charger->is_jig_on) {
		pr_info("%s: skip this function because of jig_on.(%d)\n",
			__func__, gpio_get_value(charger->pdata->jig_gpio));
		now_state = !gpio_get_value(charger->pdata->load_switch_control);
		new_state = !charger->is_jig_on;
		goto set_load_swtich;
	} else if (charger->pdata->load_switch_always_on) {
		pr_info("%s: skip check routine because of always_on.(%d)\n",
			__func__, gpio_get_value(charger->pdata->load_switch_control));

		now_state = !gpio_get_value(charger->pdata->load_switch_control);
		new_state = charger->pdata->load_switch_always_on;
		goto set_load_swtich;
	}

	/* Check Load Switch state */
	new_state = now_state = !gpio_get_value(charger->pdata->load_switch_control);
...
set_load_swtich:
	if (new_state != now_state) {
		gpio_set_value(charger->pdata->load_switch_control, !new_state);

		value.intval = new_state;
		psy_do_property(charger->pdata->main_charger_name, set,
			POWER_SUPPLY_EXT_PROP_WD_QBATTOFF, value);
	}
	...
	case SEC_MULTI_CHARGER_LOAD_SWITCH_CHARGING_OFF:
	{
		...
		if (new_state) {
			...
			else if ((charger->total_current.input_current_limit > 0) &&
					(charger->total_current.fast_charging_current > 0) &&
					(charger->float_voltage > 0)) {
				int input_power = 0, output_power = 0;
				
				psy_do_property(charger->pdata->battery_name, get,
					POWER_SUPPLY_PROP_POWER_NOW, value);

				input_power = (value.intval * 9) / 10;
				output_power =
					(charger->float_voltage * charger->total_current.fast_charging_current) / 1000;
				pr_info("%s: check power states(%d - %d)\n", __func__, input_power, output_power);
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (charger->multi_mode == SEC_MULTI_CHARGER_LOAD_SWITCH_CHARGING_OFF) {
			...
			if (charger->pdata->load_switch_control && charger->pdata->load_switch_always_on)
				value.intval = (charger->chg_mode == SEC_BAT_CHG_MODE_CHARGING_OFF) ?
					SEC_BAT_CHG_MODE_BUCK_OFF : charger->chg_mode;
	...
	{
		if (charger->pdata->load_switch_control) {
			if (charger->pdata->jig_low_active)
				charger->is_jig_on = !gpio_get_value(charger->pdata->jig_gpio);
			else
				charger->is_jig_on = gpio_get_value(charger->pdata->jig_gpio);

			gpio_set_value(charger->pdata->load_switch_control,
				(charger->is_jig_on || (!charger->pdata->load_switch_always_on)));
		}
		psy_do_property(charger->pdata->main_charger_name, set, psp, value);
		psy_do_property(charger->pdata->sub_charger_name, set, psp, value);
	}
		...
		struct sec_multi_charger_info *charger)
{
	...
	if (!np) {
		...
		ret = pdata->load_switch_control = of_get_named_gpio(np, "charger,load_switch_control", 0);
		if (ret < 0) {
			pr_err("%s: load_switch_control is Empty\n", __func__);
			pdata->load_switch_control = 0;
		} else if (pdata->load_switch_control) {
			pdata->jig_low_active = of_property_read_bool(np, "charger,jig_low_active");
			pdata->jig_gpio = of_get_named_gpio(np, "charger,jig_gpio", 0);
			if (pdata->jig_gpio < 0) {
				pr_err("%s error reading jig_gpio = %d\n",
						__func__, pdata->jig_gpio);
				pdata->jig_gpio = 0;
			}
			pdata->load_switch_always_on =
				of_property_read_bool(np, "charger,load_switch_always_on");

			if (!pdata->load_switch_always_on) {
...
static int sec_multi_charger_probe(struct platform_device *pdev)
{
	...
	if (charger->pdata->load_switch_control && charger->pdata->jig_gpio) {
		union power_supply_propval value = {0, };
		int load_switch_set_value = 0;

		if (charger->pdata->jig_low_active)
			charger->is_jig_on = !gpio_get_value(charger->pdata->jig_gpio);
		else
			charger->is_jig_on = gpio_get_value(charger->pdata->jig_gpio);
		load_switch_set_value = (charger->is_jig_on) ?
			0 : charger->pdata->load_switch_always_on;
		gpio_set_value(charger->pdata->load_switch_control, !load_switch_set_value);

		pr_info("%s: test log(%d, %d, %d, %d)\n", __func__,
			charger->is_jig_on, load_switch_set_value,
			gpio_get_value(charger->pdata->jig_gpio),
			gpio_get_value(charger->pdata->load_switch_control));

		value.intval = load_switch_set_value;
		psy_do_property(charger->pdata->main_charger_name, set,
			POWER_SUPPLY_EXT_PROP_WD_QBATTOFF, value);
	}
...
static void sec_multi_charger_shutdown(struct platform_device *pdev)
{
	...
	if (charger->pdata->load_switch_control) {
		/* Default load switch state */
		gpio_set_value(charger->pdata->load_switch_control,
			!!charger->pdata->load_switch_always_on);
	}
}
