...
int convert_ap_status(int command)
{
	int ret = -1;
	switch (command) {
	case SEMCONTEXT_AP_STATUS_SHUTDOWN :
		ret = AP_SHUTDOWN;
		break;
	case SEMCONTEXT_AP_STATUS_WAKEUP :
		ret = LCD_ON;
		break;
	case SEMCONTEXT_AP_STATUS_SLEEP :
		ret = LCD_OFF;
		break;
	case SEMCONTEXT_AP_STATUS_RESUME :
		ret = AP_RESUME;
		break;
	case SEMCONTEXT_AP_STATUS_SUSPEND :
		ret = AP_SUSPEND;
		break;
#if 0
	case SEMCONTEXT_AP_STATUS_RESET :
		ret = AP_SHUTDOWN;
		break;
#endif
	case SEMCONTEXT_AP_STATUS_POW_CONNECTED :
		ret = POW_CONNECTED;
		break;
	case SEMCONTEXT_AP_STATUS_POW_DISCONNECTED :
		ret = POW_DISCONNECTED;
		break;
	case SEMCONTEXT_AP_STATUS_CALL_IDLE :
		ret = CALL_IDLE;
		break;
	case SEMCONTEXT_AP_STATUS_CALL_ACTIVE :
		ret = CALL_ACTIVE;
		break;
	}
                     ...
                     int *receive_buf_len)
{
	...
	ssp_errf("cmd %d type %d subcmd %d send_buf_len %d timeout %d", cmd, type,
                 ...
                 u8 uSensorType, u8 *uSendBuf, u16 uLength)
{
	...
	case GO_SLEEP:
		...
		sub_cmd = SEMCONTEXT_AP_STATUS_SLEEP;
		data->uLastAPState = SEMCONTEXT_AP_STATUS_SLEEP;
		break;
	case REMOVE_LIBRARY:
		command = CMD_REMOVE;
		break;
