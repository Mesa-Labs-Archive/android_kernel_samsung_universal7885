...
static void initialize_variable(struct ssp_data *data)
{
	...
	data->uLastResumeState = SEMCONTEXT_AP_STATUS_RESUME;
...
static int ssp_prepare(struct device *dev)
{
	...
	ssp_infof();
	data->uLastResumeState = SEMCONTEXT_AP_STATUS_SUSPEND;
	disable_debug_timer(data);
	enable_irq_wake(data->irq);

	if (SUCCESS != ssp_send_status(data, SEMCONTEXT_AP_STATUS_SUSPEND)) {
		ssp_errf("SEMCONTEXT_AP_STATUS_SUSPEND failed");
	}
...
static void ssp_complete(struct device *dev) 
{
	...
	if (SUCCESS != ssp_send_status(data, SEMCONTEXT_AP_STATUS_RESUME)) {
		ssp_errf("SEMCONTEXT_AP_STATUS_RESUME failed");
	}
...
static void ssp_shutdown(struct spi_device *spi_dev)
{
	...
	if (SUCCESS != ssp_send_status(data, SEMCONTEXT_AP_STATUS_SHUTDOWN)) {
		ssp_errf("SEMCONTEXT_AP_STATUS_SHUTDOWN failed");
	}
