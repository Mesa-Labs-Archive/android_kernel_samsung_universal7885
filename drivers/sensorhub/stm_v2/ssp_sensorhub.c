                                  ...
                                  const char *buf, int count)
{
	...
	if (buf[2] < SEMCONTEXT_AP_STATUS_WAKEUP ||
	    buf[2] >= SEMCONTEXT_AP_STATUS_CALL_ACTIVE) {
		ssp_errf("INST_LIB_NOTI err(%d)", buf[2]);
		return -EINVAL;
	}

	ret = ssp_send_status(hub_data->ssp_data, buf[2]);

	if (buf[2] == SEMCONTEXT_AP_STATUS_WAKEUP ||
	    buf[2] == SEMCONTEXT_AP_STATUS_SLEEP) {
		hub_data->ssp_data->uLastAPState = buf[2];
	}

	if (buf[2] == SEMCONTEXT_AP_STATUS_SUSPEND ||
	    buf[2] == SEMCONTEXT_AP_STATUS_RESUME) {
...
#define SEMCONTEXT_VALUE_CURRENTSYSTEMTIME                      0x0E
#define SEMCONTEXT_VALUE_PEDOMETER_USERHEIGHT           0x12
#define SEMCONTEXT_VALUE_PEDOMETER_USERWEIGHT           0x13
#define SEMCONTEXT_VALUE_PEDOMETER_USERGENDER           0x14
#define SEMCONTEXT_VALUE_PEDOMETER_INFOUPDATETIME       0x15

int convert_semcontext_putvalue_subcmd(int subcmd)
{
	int ret = -1;
	switch (subcmd) {
	case SEMCONTEXT_VALUE_CURRENTSYSTEMTIME :
		ret = CURRENT_SYSTEM_TIME;
		break;
	case SEMCONTEXT_VALUE_PEDOMETER_USERHEIGHT :
		ret = PEDOMETER_USERHEIGHT;
		break;
	case SEMCONTEXT_VALUE_PEDOMETER_USERWEIGHT:
		ret = PEDOMETER_USERWEIGHT;
		break;
	case SEMCONTEXT_VALUE_PEDOMETER_USERGENDER:
		ret = PEDOMETER_USERGENDER;
		break;
	case SEMCONTEXT_VALUE_PEDOMETER_INFOUPDATETIME:
...
int convert_semcontext_getvalue_subcmd(int subcmd)
{
	int ret = -1;
	switch (subcmd) {
	case SEMCONTEXT_VALUE_CURRENTSTATUS :
		ret = LIBRARY_CURRENTSTATUS;
		break;
	case SEMCONTEXT_VALUE_CURRENTSTATUS_BATCH :
		ret = LIBRARY_CURRENTSTATUS_BATCH;
		break;
	case SEMCONTEXT_VALUE_VERSIONINFO:
                                          ...
                                          const char *buf, int count)
{
	...
	if (buf[0] == SEMCONTEXT_INST_LIBRARY_REMOVE) {
		command = CMD_REMOVE;
		type = buf[1] + SS_SENSOR_TYPE_BASE;
	} else if (buf[0] == SEMCONTEXT_INST_LIBRARY_ADD) {
		command = CMD_ADD;
		type = buf[1] + SS_SENSOR_TYPE_BASE;
	} else if (buf[0] == SEMCONTEXT_INST_LIB_SET_DATA) {
		command = CMD_SETVALUE;
		if (buf[1] != SEMCONTEXT_VALUE_LIBRARY_DATA) {
			type = TYPE_MCU;
			sub_cmd = convert_semcontext_putvalue_subcmd(buf[1]);
		} else {
	...
	} else if (buf[0] == SEMCONTEXT_INST_LIB_GET_DATA) {
		command = CMD_GETVALUE;
		type = buf[1] + SS_SENSOR_TYPE_BASE;
		sub_cmd =  convert_semcontext_getvalue_subcmd(buf[2]);
		length = count - 3;
		if (length > 0) {
			buffer = (char *)(buf + 3);
		} else {
	...
	return ssp_send_command(hub_data->ssp_data, command, type, sub_cmd, 0,
                                   ...
                                   size_t count, loff_t *pos)
{
	...
	if (buffer[0] == SEMCONTEXT_INST_LIB_NOTI) {
...
void ssp_sensorhub_report_notice(struct ssp_data *ssp_data, char notice)
{
	...
	if (notice == SEMCONTEXT_AP_STATUS_RESET) {
	...
	if (notice == SEMCONTEXT_AP_STATUS_WAKEUP) {
		ssp_infof("wake up");
	} else if (notice == SEMCONTEXT_AP_STATUS_SLEEP) {
		ssp_infof("sleep");
	} else if (notice == SEMCONTEXT_AP_STATUS_RESET) {
		ssp_infof("reset");
	} else {
