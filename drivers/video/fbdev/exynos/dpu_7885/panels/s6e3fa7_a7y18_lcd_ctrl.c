...
static int s6e3fa7_read_rdnumpe(struct lcd_info *lcd)
{
	...
	dev_info(&lcd->ld->dev, "%s: %2x\n", __func__, lcd->rdnumpe);

	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNDSIE, (lcd->rdnumpe & LDI_PNDSIE_MASK));
...
static int s6e3fa7_read_esderr(struct lcd_info *lcd)
{
	...
	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNELVDE, !!(lcd->esderr & LDI_PNELVDE_MASK));
	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNVLI1E, !!(lcd->esderr & LDI_PNVLI1E_MASK));
	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNVLO3E, !!(lcd->esderr & LDI_PNVLO3E_MASK));
	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNESDE, !!(lcd->esderr & LDI_PNESDE_MASK));
...
static int s6e3fa7_read_rddsdr(struct lcd_info *lcd)
{
	...
	s6e3fa7_inc_dpui_u32_field(lcd, DPUI_KEY_PNSDRE, !(lcd->rddsdr & LDI_PNSDRE_MASK));
...
static int s6e3fa7_init_elvss(struct lcd_info *lcd, u8 *data)
{
	...
	for (i = 0; i < IBRIGHTNESS_MAX; i++) {
		for (temp = 0; temp < TEMP_MAX; temp++) {
			...
			lcd->elvss_table[i][temp][0] = elvss_mpscon_data[i][0];
			lcd->elvss_table[i][temp][LDI_OFFSET_ELVSS_1] = NORMAL_TEMPERATURE;				/* B5h 1st Para: TSET */
			lcd->elvss_table[i][temp][LDI_OFFSET_ELVSS_2] = elvss_mpscon_data[i][LDI_OFFSET_ELVSS_2];	/* B5h 2nd Para: MPS_CON */
...
static int s6e3fa7_read_init_info(struct lcd_info *lcd)
{
	...
	ret |= s6e3fa7_read_mtp(lcd);
	ret |= s6e3fa7_read_coordinate(lcd);
	ret |= s6e3fa7_read_chip_id(lcd);
	ret |= s6e3fa7_read_elvss(lcd, elvss_data);
	ret |= s6e3fa7_read_manufacture_info(lcd);
	ret |= s6e3fa7_read_hbm(lcd);
	ret |= s6e3fa7_read_irc(lcd, irc_data);
	ret |= s6e3fa7_init_elvss(lcd, elvss_data);
	ret |= s6e3fa7_init_hbm_elvss(lcd, elvss_data);
	ret |= s6e3fa7_init_irc(lcd, irc_data);
